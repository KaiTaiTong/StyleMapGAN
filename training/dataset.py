...
class GTMaskDataset(Dataset):
    ...
    def __init__(self, lmdb_folder, transform, resolution=256):
        ...
        original_path = f"{dataset_folder}/raw_images/images"
        ...
        with open(CelebA_HQ_to_CelebA, "r") as fp:
            ...
            while True:
                ...
                idx, orig_idx, orig_file = (re.sub(
                    " +", " ", read_line).strip().split(" "))
...
class DataSetTestLocalEditing(Dataset):
    ...
    def __getitem__(self, idx):
        image_mask = self.transform(
            Image.open(
                os.path.join(self.main_dir, "mask",
                             self.total_imgs[idx])).convert("RGB"))
        image_reference = self.transform(
            Image.open(
                os.path.join(self.main_dir, "reference_image",
                             self.total_imgs[idx])).convert("RGB"))
        ...
        image_source = self.transform(
            Image.open(
                os.path.join(self.main_dir, "source_image",
                             self.total_imgs[idx])).convert("RGB"))
        ...
        image_synthesized = self.transform(
            Image.open(
                os.path.join(self.main_dir, "synthesized_image",
                             self.total_imgs[idx])).convert("RGB"))
